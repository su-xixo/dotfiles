...
H.create_autocommand = function()
	...
	au("MiniFilesExplorerOpen", function() -- bookmarks, alternative is visits_harpooned
		files.set_bookmark("c", vim.fn.stdpath("config") .. "", { desc = "Neovim config" })
		files.set_bookmark("w", vim.fn.getcwd, { desc = "Working directory" })
		files.set_bookmark("C", vim.fn.expand("~/.config"), { desc = "Config directory" })
		if vim.fn.isdirectory(vim.fn.expand("~/dotfiles")) ~= 0 then
			files.set_bookmark("d", vim.fn.expand("~/dotfiles"), { desc = "Dotfiles directory" })
		end
    ...
    -- stylua: ignore start
    map_buf("<CR>", function() files.go_in({ close_on_file = true }) end)
    map_buf("<Right>", function() files.go_in({ close_on_file = true }) end)

    map_buf("<BS>", function() files.go_out() end)
    map_buf("<Left>", function() files.go_out() end)

    map_buf("<Esc>", function() files.close() end)
